...
def pydeseq2Test(X, y, z, nc):
    """
    X: count matrix of genes (m genes X n samples)
...
def get_options():
    ...
    parser.add_argument("-x", dest="gene_table",
                       help="count matrix of genes (m genes X n samples)",
                       required=True)
...
def main():
    option = get_options()
    input_table = pd.read_table(option.gene_table, sep = '\t', index_col = 0)
    ...
    res_df = pydeseq2Test(X = input_table,
                                                y = input_table2,
